webshims.register("form-validation", function(a, b, c, d, e, f) {
    ...
    function h() {
        if (!a.data(this, "wsCustomFile")) {
            ...
            a.data(this, "wsCustomFile", {
                showSelected: h
            }), a("button:not(.ws-capture-button)", c).attr("tabindex", "-1"), d.on("change.webshim", h).each(h).jProp("form").on("reset", function() {
                setTimeout(h)
    ...
    var i = "webkitURL" in c,
        ...
        M = function(c) {
            if (p.sel) {
                var d, e, f, g;
                if (c.target && (d = a(c.target).getNativeElement()[0], f = a(d).getShadowElement(), "submit" != d.type && a.prop(d, "willValidate") && ("change" != c.type || !(g = f.prop("type")) || I[g]))) {
    ...
    "validityUIEvents" in f && (b.error("validityUIEvents was renamed to iVal.events"), p.events = f.validityUIEvents), p.events = "events" in p ? p.events || "" : "focusout change", p.events && (p.events += " "), p.fieldWrapper || (p.fieldWrapper = ":not(span):not(label):not(em):not(strong):not(p):not(.ws-custom-file)"), E["form-core"].getGroupElements || (E["form-core"].getGroupElements = F), a(d.body || "html").on(p.events + "refreshvalidityui updatevalidation.webshims invalid", M).on("refreshvalidationui.webshims", function(b) {
        a(b.target).getShadowElement().is(s) && M({
            type: "updatevalidation",
            target: b.target
        })
    }).on("reset resetvalidation.webshims resetvalui", function(c) {
        var d, e = a(c.target);
        "resetvalui" == c.type && b.error("resetvalui was renamed to resetvalidation"), e.is("form, fieldset") && ("form" == e[0].nodeName.toLowerCase() && (d = !e.is(p.sel)), e = e.jProp("elements")), e = e.filter(L).removeAttr("aria-invalid").removeClass(p.errorClass + " " + p.successClass).getNativeElement().each(function() {
    ...
    b.getRelOffset = function(b, c, d) {
        var e, f;
        return b = a(b), a.swap(b[0], Q, function() {
            var g;
            a.position && d && a.position.getScrollInfo ? (d.of || (d.of = c), g = "rtl" == a(d.of).css("direction"), d.isRtl || (d.isRtl = !1), d.isRtl != g && (d.my = (d.my || "center").replace(S, T), d.at = (d.at || "center").replace(S, T), d.isRtl = g), b[d.isRtl ? "addClass" : "removeClass"]("ws-is-rtl"), d.using = function(a, c) {
                b.attr({
        ...
        _getAutoAppendElement: function() {
            ...
            return function(c) {
                for (var e, f = c[0], g = d.body;
                    (f = f[e ? "offsetParent" : "parentNode"]) && 1 == f.nodeType && f != g;) e || b.test(f.nodeName) || (e = f), e && "visible" != a.css(f, "overflow") && "static" != a.css(f, "position") && (e = !1);
                return a(e || g)
            }
        }(),
    ...
    }(), p.fx && R[p.fx] || (p.fx = "slide"), a.fn[R[p.fx].show] || (p.fx = "no");
    var U = 0;
    b.errorbox = {
            create: function(b, c) {
                c || (c = this.getFieldWrapper(b));
                var d = a("." + v, c);
                return d.length || (d = a("<" + x + ' class="' + v + '" hidden="hidden" style="display: none;" />'), c.append(d)), d.prop("id") || (U++, d.prop("id", "errorbox-" + U)), c.data("errorbox", d), d
            },
            ...
            hideError: function(b, c) {
                ...
                return c || (d = a("input:invalid, select:invalid, textarea:invalid", g)), g.hasClass(t) && (a(b).filter("input").off(".recheckinvalid"), !c && d && d[0] ? a(d).trigger("updatevalidation.webshims") : (e = this.get(b, g), g.removeClass(t), e.message = "", f = function() {
            ...
            showError: function(b) {
                ...
                return d.message != e && (d.stop && d.stop(!0, !0), d.html("<" + y + ' class="' + w + '">' + e + "</" + y + ">"), d.message = e, c.addClass(t).removeClass(u), this.recheckInvalidInput(b), (d.is("[hidden]") || "none" == d.css("display")) && (b.getAttribute("aria-describedby") || b.setAttribute("aria-describedby", d.prop("id")), d.css({
                    display: "none"
                }).removeAttr("hidden")[R[p.fx].show]())), c.removeClass(u), a(b).off(".recheckvalid"), c
            },
            reset: function(a) {
        ...
        function() {
            ...
            a(d).on("invalid", function(d) {
                if (!d.wrongWebkitInvalid && !e) {
                    var g = a(d.target);
                    b || a.find.matchesSelector(d.target, "form, fieldset") || (b = a.Event("firstinvalid"), g.addClass("first-invalid").trigger(b)), b && b.isDefaultPrevented() && d.preventDefault(), f.push(d.target), d.extraData = "fix", clearTimeout(c), c = setTimeout(function() {
                        var c = {
                            type: "lastinvalid",
                            cancelable: !1,
                            invalidlist: a(f)
        ...
        }), a.swap || (a.swap = function(a, b, c, d) {
            var e, f, g = {};
